...
class ReceiverReport {
    ...
    constructor(buffer) {
        ...
        if (!buffer) {
            this.buffer = Buffer.alloc(exports.REPORT_LENGTH);
            return;
        }
        if (buffer.length < exports.REPORT_LENGTH) {
            throw new TypeError('buffer is too small');
        }
        this.buffer = buffer.slice(undefined, exports.REPORT_LENGTH);
    }
    /**
     * @ignore
     */
 ...
 */
class ReceiverReportPacket extends _1.RtcpPacket {
    /**
    ...
    constructor(buffer) {
        ...
        if (buffer.length < FIXED_HEADER_LENGTH + (count * exports.REPORT_LENGTH)) {
            throw new TypeError('buffer is too small');
        }
        while (count-- > 0) {
            const report = new ReceiverReport(buffer.slice(FIXED_HEADER_LENGTH + (this.reports.length * exports.REPORT_LENGTH)));
            this.addReport(report);
        }
        // Store a buffer within the packet boundaries.
        this.buffer = buffer.slice(undefined, FIXED_HEADER_LENGTH + (this.reports.length * exports.REPORT_LENGTH) + this.padding);
    }
    /**
     * @ignore
     */
    ...
    serialize() {
        ...
        const length = FIXED_HEADER_LENGTH + (exports.REPORT_LENGTH * this.reports.length);
        ...
        for (let i = 0; i < this.reports.length; ++i) {
            ...
            report.getBuffer().copy(this.buffer, FIXED_HEADER_LENGTH + (exports.REPORT_LENGTH * i));
        }
...
// Packet Type.
ReceiverReportPacket.packetType = _1.RtcpPacketType.RR;
