...
class RtpPacket {
    ...
    constructor(buffer) {
        ...
        if (extFlag) {
            ...
            extBuffer = Buffer.from(buffer.buffer, buffer.byteOffset + pos + 4, length);
            pos += (4 + length);
        }
        ...
        this.payload = Buffer.from(buffer.buffer, buffer.byteOffset + pos, payloadLength);
    ...
    setPayloadType(payloadType) {
        this.buffer.writeUInt8((this.buffer.readUInt8(1) & 0x80) | (payloadType & 0x7F), 1);
    }
    /**
    ...
    setCsrc(csrc = []) {
        ...
        this.buffer.writeUInt8((this.buffer.readUInt8(0) & 0xF0) | (count & 0x0F), 0);
    }
    /**
    ...
    padTo4Bytes() {
        ...
        const newLength = utils_1.padTo4Bytes(length - padding);
        if (newLength === length) {
            return;
        }
